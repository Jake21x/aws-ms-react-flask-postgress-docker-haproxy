...
class ApiGetCategory(Resource):
    def get(self):
        ...
        for c in chain(range(0, len(x))):
            refs = conn.execute("SELECT brand, refsid as tblrefid,catid as tblcategoryid, segment,\'NA\' as subsegment, percent_share, facing_count, pulloutday,facing_brand,facing_segment FROM category_refs WHERE catid = '{}'".format(x[c]['tblcategoryid']),result=True)
